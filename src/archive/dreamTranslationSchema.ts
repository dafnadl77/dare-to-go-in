...
export type TranslationTarget = 'en' | 'he';

const TARGET_NAME: Record<TranslationTarget, string> = { en: 'English', he: 'Hebrew' };

/** `target` is the language of the interface the passages will be shown in
    (English until Hebrew could be a UI language too — the default). */
export function buildDreamTranslationSystemPrompt(target: TranslationTarget = 'en'): string {
  const name = TARGET_NAME[target];
  return `You translate real passages from a personal dream journal into natural, fluent ${name}, for a product interface that is entirely in ${name} regardless of what language the dream was originally described in.

You will be given a numbered list of REAL passages, each already part of one real dreamer's own saved dream. Each passage may already be in ${name} (translate it if not) or in any other language.

Rules:
- Read the ACTUAL TEXT of each numbered item and translate it faithfully into natural, fluent ${name}. Preserve the original meaning, tone, and level of detail — this is a translation, not a summary or condensation. A full sentence stays a full sentence; a short phrase stays a short phrase.
- If an item is already in ${name}, return it unchanged (correct only obvious typos, do not rephrase or paraphrase it).
...
${buildLanguageIntegrityInstruction(target)}`;
}

export const DREAM_TRANSLATION_SYSTEM_PROMPT = buildDreamTranslationSystemPrompt('en');
